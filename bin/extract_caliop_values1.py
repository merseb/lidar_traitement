...
path = os.getcwd() + '/lidar_traitement'
...
for k in sorted(dt_nday.keys())[idt+2:idt+3]:
    ...
    for day in dt_nday[k][:]:
        ...
        for f in files[day][:]:
            ...
            #####
            #df_file.drop(['Layer_Base_Altitude', 'Layer_Top_Altitude', 'Feature_Classification_Flags'], inplace=True)
            #####
        ...
        try:
            for subtype in df_day.FeatureSubtype.unique().tolist():
                ...
                df_day.nb_values_window_1deg[df_day.FeatureSubtype == subtype ] = [calcul_Nbpoints(matrice_latitude, vx) for vx in list(matrice_latitude)]
            ...
            lp = list(set(df_day.columns) - set(['Layer_Base_Altitude', 'Layer_Top_Altitude', 'Feature_Classification_Flags']))
